...
import * as sync from "./src/server/sync";
...
const DEFAULT_SHOP_SETTINGS = {
  ...
  whatsapp_notify_on_ready: false,
...
async function getAccountSettings(userSupabase: any, userId: string): Promise<Record<string, any>> {
  ...
  const criticalKeys = ["shop_name", "phone", "address", "currency", "pipeline_stages", "measurement_fields", "auto_archive_days", "measurement_unit", "terms_conditions", "receipt_footer_text", "default_print_receipt", "default_print_measure", "whatsapp_notify_on_ready", "whatsapp_message_template"];
  ...
  if (!settingsMap.whatsapp_message_template) {
    settingsMap.whatsapp_message_template = DEFAULT_SHOP_SETTINGS.whatsapp_message_template;
  }
  ...
  try {
    if (useLocalDb()) {
      ...
      const customer = db.createCustomer({
        name, phone: dbPhone, whatsapp: whatsapp || null, address: address || null,
  ...
  try {
    if (useLocalDb()) {
      ...
      const order = db.createOrder({
        ...
        total_amount, paid_amount, due_date, measurement_snapshot: snapshot,
        shop_id: req.user!.shop_id, created_by: req.user!.id, updated_by: req.user!.id,
      });
...
app.post("/api/workers", requireAuth, requireRole(["Owner"]), async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (useLocalDb()) {
      ...
      const { error: profErr } = await supabaseAdmin.from("profiles").insert([{ ...newProfile, created_at: now, updated_at: now }]);
      if (profErr) {
