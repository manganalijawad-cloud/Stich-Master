...
const DEFAULT_SHOP_SETTINGS = {
  ...
  terms_conditions: "",
  receipt_footer_text: "Receipt is generated by Stitch Master - 03163455358",
  default_print_receipt: true,
  default_print_measure: true,
  whatsapp_message_template: `{ShopName}
...
async function getAccountSettings(userSupabase: any, userId: string): Promise<Record<string, any>> {
  ...
  const criticalKeys = ["shop_name", "phone", "address", "currency", "pipeline_stages", "measurement_fields", "auto_archive_days", "measurement_unit", "terms_conditions", "receipt_footer_text", "default_print_receipt", "default_print_measure", "whatsapp_message_template"];
...
app.delete("/api/workers/:id", requireAuth, requireRole(["Owner"]), async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    // Clear references in other tables before deleting the user
    ...
    // Delete profile first (FK allows this, it only restricts INSERT)
    ...
    // Delete from auth.identities if accessible (Supabase internal)
    try {
      await supabaseAdmin.from("auth.identities").delete().eq("id", workerId);
    } catch (e) { /* identities table may not be accessible */ }
...
app.post("/api/import/customers", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (create_measurements && garment_type_id) {
    const { data: gt } = await userSupabase
      .from("garment_types")
      .select("measurement_fields")
      .eq("id", garment_type_id)
      .maybeSingle();
    if (gt?.measurement_fields) {
      knownFields = gt.measurement_fields.map((f: any) => f.name || f);
    }

    if (knownFields.length === 0) {
      const { data: settings } = await userSupabase
        .from("shop_settings")
        .select("value")
        .eq("key", `${req.user!.id}:garment_types`)
        .maybeSingle();
      if (settings?.value) {
  ...
  for (const row of customers) {
    try {
      ...
      if (cleanPhone) {
        ...
        if (existing) {
          results.skipped++;
          results.details.push({ name: customerData.name, status: "skipped", reason: `Duplicate phone: ${cleanPhone}` });
          continue;
        }
      }
      ...
      const { data: nameMatch } = await userSupabase
        ...
        .select("id")
        .eq("created_by", req.user!.id)
        .ilike("name", String(customerData.name).trim());
      if (nameMatch && nameMatch.length > 0 && !cleanPhone) {
        results.skipped++;
        results.details.push({ name: customerData.name, status: "skipped", reason: "Duplicate name, no phone." });
        continue;
      }
      ...
      const { data: created, error: custErr } = await userSupabase
        ...
        .insert([{
          ...
          phone: dbPhone,
          whatsapp: customerData.whatsapp ? String(customerData.whatsapp).trim() : null,
